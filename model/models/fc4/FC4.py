...
class FC4(torch.nn.Module):
    ...
    def forward(self, x: Tensor) -> Union[tuple, Tensor]:
        ...
        if USE_CONFIDENCE_WEIGHTED_POOLING:
            ...
            # torchvision.utils.save_image(rgb, 'fig_results/vis/pi.png')

            # Confidence (last dimension)
            confidence = out[:, 3:4, :, :]
            # torchvision.utils.save_image(confidence, 'fig_results/vis/ci.png', normalize=True)
            # confidence = normalize(confidence, dim=1)
            # print(confidence)

            # Confidence-weighted pooling
            pred = normalize(torch.sum(torch.sum(rgb * confidence, 2), 2), dim=1)

            # torchvision.utils.save_image(rgb * confidence, 'fig_results/vis/map.png')
